...
const DocumentUploadAndVerification = () => {
  ...
  const validateForm = () => {
    ...
    if (!files?.father && !files?.mother) {
      newErrors.general = 'At least one parent passport (Father or Mother) is required';
      isValid = false;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (serverStatus === 'offline') {
      setErrors(prev => ({
        ...prev,
        general: 'Unable to connect to verification server. Please check if the server is running on http://localhost:8000'
      }));
      return;
    }
  ...
  const handleDownload = () => {
    ...
    const exportFileDefaultName = `verification-results-${new Date()?.toISOString()?.split('T')?.[0]}.json`;
  ...
  const canSubmit = files?.student && files?.translated && (files?.father || files?.mother) && serverStatus === 'online';
      ...
      <main className="container mx-auto px-6 py-8 max-w-4xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Upload family passport documents and translated materials for comprehensive verification analysis. 
            Our system will compare information across all documents and provide detailed matching results.
          </p>
          ...
          <div className="flex items-center justify-center mt-4">
            <div className={`flex items-center px-3 py-1 rounded-full text-sm ${
              serverStatus === 'online' ?'bg-success/10 text-success' 
                : serverStatus === 'offline' ?'bg-error/10 text-error' :'bg-warning/10 text-warning'
            }`}>
              <div className={`w-2 h-2 rounded-full mr-2 ${
                serverStatus === 'online' ?'bg-success animate-pulse' 
                  : serverStatus === 'offline' ?'bg-error' :'bg-warning animate-pulse'
              }`}></div>
            ...
            <div className="space-y-8">
              ...
              {serverStatus === 'offline' && (
                <div className="flex items-center p-4 bg-warning/10 border border-warning/20 rounded-lg">
                  <Icon name="AlertTriangle" size={20} className="text-warning mr-3 flex-shrink-0" />
                  <div className="flex-1">
                    <div className="text-sm text-warning font-medium">Server Connection Issue</div>
                    <div className="text-xs text-warning/80 mt-1">
                      Cannot connect to verification server at http://localhost:8000. 
              ...
              <FileUploadZone
                ...
                required
                file={files?.student}
                onFileSelect={(file, error) => handleFileSelect('student', file, error)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FileUploadZone
                  ...
                  file={files?.father}
                  onFileSelect={(file, error) => handleFileSelect('father', file, error)}
                  error={errors?.father}
                  disabled={isLoading || serverStatus === 'offline'}
                />
                ...
                <FileUploadZone
                  ...
                  file={files?.mother}
                  onFileSelect={(file, error) => handleFileSelect('mother', file, error)}
                  error={errors?.mother}
                  disabled={isLoading || serverStatus === 'offline'}
                />
              </div>
              ...
              <FileUploadZone
                ...
                onFileSelect={(file, error) => handleFileSelect('translated', file, error)}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-border space-y-4 sm:space-y-0">
                ...
                <div className="flex items-center space-x-3">
                  <Button
                    ...
                    iconPosition="left"
                    onClick={handleReset}
                    disabled={isLoading || (!files?.student && !files?.father && !files?.mother && !files?.translated)}
                  >
                    Reset Form
                  </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
            <div className="bg-card border border-border rounded-lg p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                All documents are processed securely and deleted after verification
              </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-lg p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Comprehensive field-by-field comparison with detailed status reports
              </p>
            </div>
          </div>
        </div>
